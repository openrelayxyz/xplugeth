...
func init() {
	xplugeth.RegisterModule[beaconAgentModule]("beaconAgentModule")
}
...
func (*beaconAgentModule) InitializeNode(s *node.Node, b types.Backend) {
	...
	log.Info("beacon agent plugin initialised")
	...
	for {
		select {
		case <-sigs:
			log.Info("received shutdown signal, closing beacon agent plugin")
			sm.Close()
			time.Sleep(30)
			return
		case err := <-errCh:
