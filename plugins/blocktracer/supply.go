...
func init() {
	log.Error("xplugeth supply tracer")
	tracers.LiveDirectory.Register("xsupply", xnewSupply)
}
...
func xnewSupply(cfg json.RawMessage) (*tracing.Hooks, error) {
	...
	log.Error("inside of xplugeth new supply")
	config.Path = "./test"
	if config.Path == "" {
